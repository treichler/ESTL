...
/**
 * Table properties
 * Must not exceed 7, since 3 bits reserved
 */
enum {
  TABLE_PROPERTY_INDEX_RANGE    = 0,
  TABLE_PROPERTY_TABLE_CRC      = 1,    //!<
};
 ...
 */
enum {
  OFFSET_PARAM_ACTUAL   = 0,    //!<
  OFFSET_PARAM_NOMINAL  = 1,    //!<
  OFFSET_PARAM_MINIMUM  = 2,    //!<
  OFFSET_PARAM_MAXIMUM  = 3,    //!<
  OFFSET_PARAM_PROPERTY = 4,    //!< Combined access to fetch three properties: Unit, Representation and Flags
  OFFSET_PARAM_NAME     = 5,    //!< Access Parameter name
  OFFSET_PARAM_INFO     = 6,    //!< Access Parameter info (help text).
};

#ifndef CANOPEN_PARAMETER_SDO_START_INDEX
#define CANOPEN_PARAMETER_SDO_START_INDEX     (0x5FE0)
#warning "CANOPEN_PARAMETER_SDO_START_INDEX is set to default value 0x5FE0."
#endif

#define CANOPEN_PARAMETER_SDO_LAST_ADDRESS      (CANOPEN_PARAMETER_SDO_START_INDEX + PARAMETER_SDO_AMOUNT_OF_SDO_INDICES)
#if( (CANOPEN_PARAMETER_SDO_START_INDEX < 0x2000) || (CANOPEN_PARAMETER_SDO_LAST_ADDRESS > 0x5FFF) )
#error "CANOPEN_PARAMETER_SDO_START_ADDRESS violates range 0x2000..0x5FFF (defined in ESTL_Config.h)."
#endif
...
error_code_t ParameterSdo_ReadTableIndexRange( uint8_t node_id, range_t * parameter_index_range )
{
  ...
  Sdo_ExpRead( node_id, SDO_INDEX(PARAMETER_TABLE_PROPERTY_INDEX), SDO_SUBINDEX(PARAMETER_TABLE_PROPERTY_INDEX) + TABLE_PROPERTY_INDEX_RANGE, &temp, 0 );
  while( Sdo_ReqIsBusy() );
...
{
  Sdo_ExpRead( node_id, SDO_INDEX(PARAMETER_TABLE_PROPERTY_INDEX), SDO_SUBINDEX(PARAMETER_TABLE_PROPERTY_INDEX) + TABLE_PROPERTY_TABLE_CRC, (int32_t*)crc, 0 );
  while( Sdo_ReqIsBusy() );
...
{
  Sdo_SegRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_NAME, name, len );
  while( Sdo_ReqIsBusy() );
  return Sdo_ReqFinishStatus();
}
...
error_code_t ParameterSdo_ReadInfo( uint8_t node_id, int16_t parameter_index, char * info, uint16_t len )
{
  if( len <= 1 )
    return -1;
  Sdo_SegRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_INFO, info, len-1 );
  while( Sdo_ReqIsBusy() );
  info[len-1] = '\0';
  return Sdo_ReqFinishStatus();
}
...
{
  Sdo_ExpRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_ACTUAL, value, 0 );
  while( Sdo_ReqIsBusy() );
  return Sdo_ReqFinishStatus();
}
...
error_code_t ParameterSdo_WriteValue( uint8_t node_id, int16_t parameter_index, int32_t value )
{
  ...
  Sdo_ExpWrite( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_ACTUAL, value, 4 );
  while( Sdo_ReqIsBusy() );
  req_status = Sdo_ReqFinishStatus();
...
error_code_t ParameterSdo_ReadTableEntry( uint8_t node_id, int16_t parameter_index, parameter_data_t * parameter_data )
{
  ...
  Sdo_ExpRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_NOMINAL, &temp, 0 );
  while( Sdo_ReqIsBusy() );
  req_status = Sdo_ReqFinishStatus();
  if( OK != req_status )
    return req_status;
  ...
  Sdo_ExpRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_MINIMUM, &temp, 0 );
  while( Sdo_ReqIsBusy() );
  req_status = Sdo_ReqFinishStatus();
  if( OK != req_status )
    return req_status;
  ...
  Sdo_ExpRead( node_id, SDO_INDEX(parameter_index), SDO_SUBINDEX(parameter_index) + OFFSET_PARAM_MAXIMUM, &temp, 0 );
  while( Sdo_ReqIsBusy() );
  req_status = Sdo_ReqFinishStatus();
  if( OK != req_status )
    return req_status;
...
uint8_t ParameterSdo_CallbackSdoReq( uint8_t length_req, uint8_t *req_ptr, uint8_t *length_resp, uint8_t *resp_ptr )
{
  *length_resp = 8;
  if( 8 == length_req )
  {
    ...
    int16_t  parameter_index = (int16_t)((((index - CANOPEN_PARAMETER_SDO_START_INDEX) << 5) | (req_ptr[3] >> 3)) + PARAMETER_TABLE_PROPERTY_INDEX );
    uint8_t  parameter_request = req_ptr[3] & 0x07;
    ...
    {
      if( (index >= CANOPEN_PARAMETER_SDO_START_INDEX) && (index <= CANOPEN_PARAMETER_SDO_LAST_ADDRESS) )
      {
        // request to parameter
        uint32_t payload;
        uint8_t len;
        range_t index_range;

        // check if request for table properties or parameter properties
        if( PARAMETER_TABLE_PROPERTY_INDEX == parameter_index )
        {
          // request for table properties
          switch( parameter_request )
          {
            case TABLE_PROPERTY_INDEX_RANGE:
              index_range = Parameter_GetIndexRange();
              payload = (index_range.max << 16) | (index_range.min & 0xFFFF);
              len = sizeof(index_range);
              break;

            case TABLE_PROPERTY_TABLE_CRC:
        ...
        {
          // request for parameter properties
          ...
          if( (OFFSET_PARAM_NAME == parameter_request) || (OFFSET_PARAM_INFO == parameter_request) )
          {
            // segmented read
            read_ofs = 0;

            if( OFFSET_PARAM_NAME == parameter_request )
        ...
        // expedited response with 1..4 [len] data bytes
    ...
    else if( ((req_ptr[0] & 0xE0) == 0x20) && (index >= CANOPEN_PARAMETER_SDO_START_INDEX) && (index <= CANOPEN_PARAMETER_SDO_LAST_ADDRESS) && (OFFSET_PARAM_ACTUAL == parameter_request) ) // expedited write
    {
